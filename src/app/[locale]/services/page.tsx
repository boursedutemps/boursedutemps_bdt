  ...
  const handleRequest = async () => {
    ...
    try {
      const res = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({ toId: selected.user_id, serviceTitle: selected.title, amount: selected.credit_cost }),
      })
    ...
    <main className="min-h-screen bg-[#FFFCF7] pt-24 pb-16 px-4">
              ...
              <div key={service.id} className="bg-white rounded-2xl p-5 border border-slate-100 hover:border-amber-200 hover:shadow-md transition-all duration-200 flex flex-col">
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between gap-2">
                  ...
                  {!user ? (
                    ...
                    <span className="text-xs text-slate-400 italic flex-shrink-0">{t('yourService')}</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {selected && (
        <div role="dialog" aria-modal="true" aria-labelledby="modal-title"
          ...
          <div className="bg-white text-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <h2 id="modal-title" className="text-lg font-bold text-slate-800 mb-1">{t('confirm.title')}</h2>
            <p className="text-sm text-slate-500 mb-5">
              {t('confirm.subtitle')} <strong className="text-slate-700">{selected.user_name}</strong>.
            </p>
            <div className="bg-amber-50 rounded-xl p-4 mb-5 space-y-2.5">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">{t('confirm.serviceLabel')}</span>
                <span className="font-semibold text-slate-800 text-right max-w-[55%] leading-tight">{selected.title}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm border-t border-amber-100 pt-2.5">
                ...
                <span className={`font-bold ${(user?.credits ?? 0) >= selected.credit_cost ? 'text-green-700' : 'text-red-600'}`}>
                  {user?.credits ?? '—'}
                </span>
              </div>
            </div>
