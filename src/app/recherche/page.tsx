...
const SUGGESTIONS = [
  ...
  { icon: '🔍', text: 'Compare React, Vue et Angular en 2025' },
];
...
function SearchContent() {
  ...
  useEffect(() => {
    setMessages([{
      role: 'assistant',
      content: `Bonjour${user ? ` ${user.firstName}` : ''} 👋 Je suis **ALDÉA**, votre assistante IA avancée.\n\nIci, j'utilise automatiquement le meilleur modèle selon votre question :\n• 🟠 **Groq** — questions rapides\n• 🟣 **Claude** — analyse & raisonnement\n• 🟢 **GPT-4o** — créativité & rédaction\n• 🔵 **Gemini** — multimodal & Google\n• 🩷 **Kimi** — contexte long\n• 🔵 **Perplexity** — recherche web temps réel\n\nComment puis-je vous aider ?`,
      timestamp: new Date(),
    }]);
  }, [user]);

  // Pré-remplir depuis le chatbot flottant (?q=...)
  useEffect(() => {
  ...
  const clearChat = () => setMessages([{
    role: 'assistant', content: 'Conversation effacée. Comment puis-je vous aider ?', timestamp: new Date(),
  }]);
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => router.back()} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition text-slate-400">
            <ArrowLeft size={18} />
          </button>
          <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 dark:shadow-none">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportChat} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-700 rounded-xl transition" title="Exporter">
            <Download size={16} />
          </button>
          <button onClick={clearChat} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 rounded-xl transition" title="Effacer">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-6">
            {SUGGESTIONS.map((s, i) => (
              <button key={i} onClick={() => sendMessage(s.text)}
                className="flex items-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-300 p-3 rounded-2xl text-left transition group shadow-sm hover:shadow-md"
              >
                <span className="text-xl">{s.icon}</span>
                <span className="text-xs text-slate-600 dark:text-slate-300 group-hover:text-blue-600 font-medium leading-tight">{s.text}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div className="flex-shrink-0 pt-4 border-t border-slate-100 dark:border-slate-700">
        <div className="flex gap-3 items-end bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-2xl px-4 py-3 shadow-sm focus-within:ring-2 focus-within:ring-blue-500 transition">
          <textarea
            ...
            className="flex-1 bg-transparent outline-none text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 disabled:opacity-50 max-h-32 leading-relaxed"
          />
          <button
            onClick={() => sendMessage()}
        ...
        <p className="text-[10px] text-slate-400 text-center mt-2">
          Routage automatique · Groq · Claude · GPT-4o · Gemini · Kimi · Perplexity via OpenRouter
        </p>
      </div>
    </div>
  );
