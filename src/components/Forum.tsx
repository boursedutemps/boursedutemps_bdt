      ...
      {showAdd && (
        <div className="mb-12 bg-white p-8 rounded-[2rem] border border-blue-100 shadow-xl shadow-blue-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <input required placeholder="Titre du sujet"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input type="url" placeholder="Lien externe (optionnel)"
                className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 outline-none focus:ring-2 focus:ring-blue-500 transition"
          ...
          <div key={topic.id} className="bg-white p-8 rounded-[2rem] border border-slate-100 hover:border-blue-200 transition group relative">
            {user && (user.uid === topic.authorId || user.role === 'admin') && (
              <div className="absolute top-4 right-4 flex gap-2">
                <button onClick={() => { setEditingPost(topic); setNewTitle(topic.title ?? ''); setNewMsg(topic.message ?? ''); setExternalLink(topic.externalLink || ''); if (topic.media?.[0]) { setMediaData(topic.media[0].url); setMediaType(topic.media[0].type); } setShowAdd(true); }}
                  className="p-2 text-slate-400 hover:text-blue-600 transition bg-white/80 rounded-full"><Edit2 size={16} /></button>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-grow min-w-0">
                ...
                <div className="flex items-center gap-6 border-t border-slate-50 pt-4">
                  ...
                  {/* Partager — ShareMenu avec réseaux sociaux */}
                  <ShareMenu
