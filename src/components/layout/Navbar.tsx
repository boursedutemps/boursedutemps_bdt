    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/50 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            <Link href="/recherche" className={`px-3 py-2 rounded-full text-sm font-semibold transition-all duration-200 flex items-center gap-1.5 ${
              pathname === '/recherche'
                ...
                : 'text-blue-600 hover:text-white hover:bg-gradient-to-r hover:from-blue-600 hover:to-purple-600 border border-blue-200 hover:border-transparent'
            }`}>
            ...
            <div className="ml-4 pl-4 border-l border-slate-200 flex items-center gap-2">
              ...
              <div className="relative" ref={searchRef}>
                ...
                {isSearchOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-2">
                    ...
                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
                      ...
                      <div className="max-h-96 overflow-y-auto">
                        {notifications.length > 0 ? notifications.map((n) => (
                          <div key={n.id} onClick={() => { onMarkRead(n.id); if (n.type === 'message' || n.type === 'connection') router.push('/profile'); setShowNotifications(false); }}
                            className={`p-4 border-b border-slate-50 cursor-pointer transition hover:bg-slate-50 ${!n.isRead ? 'bg-blue-50/30' : ''}`}>
                            <div className="flex gap-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-lg ${n.type === 'transaction' ? 'bg-green-100' : n.type === 'connection' ? 'bg-purple-100' : 'bg-blue-100'}`}>
                                {n.type === 'transaction' ? '💰' : n.type === 'connection' ? '🤝' : '🔔'}
                              </div>
                              <div className="space-y-1">
                                <p className="text-sm text-slate-700 leading-tight"><span className="font-bold">{n.fromName}</span> {n.content}</p>
                          ...
                          <div className="p-8 text-center"><p className="text-sm text-slate-400">Aucune notification</p></div>
                        )}
                      </div>
                    </div>
                  )}
                ...
                <div className="relative" ref={userMenuRef}>
                  ...
                  >
                    ...
                    <div className="w-7 h-7 rounded-full bg-white flex items-center justify-center overflow-hidden border border-blue-400 shadow-sm group-hover:scale-105 transition-transform relative">
                      {user.avatar ? (
                        <Image src={user.avatar} alt="Avatar" fill className="object-cover" unoptimized={user.avatar.startsWith('data:')} sizes="28px" quality={80} />
                      ) : (
          ...
          {/* Mobile Menu Button */}
          <button className="lg:hidden p-2 text-slate-600 hover:bg-slate-50 rounded-full transition-colors" onClick={() => setIsOpen(!isOpen)}>
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-slate-100 py-4 px-4 space-y-1 shadow-2xl">
          {navItems.map((item) => (
            <Link key={item.page} href={item.path} onClick={() => setIsOpen(false)}
              className={`w-full text-left px-4 py-3 rounded-xl text-base font-bold transition-colors block ${pathname === item.path ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              {item.label}
            </Link>
          ))}
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 space-y-3">
            {user ? (
              ...
              <button onClick={() => { onLogin(); setIsOpen(false); }}
