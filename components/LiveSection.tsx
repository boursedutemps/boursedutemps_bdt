...
import { Radio, Plus, X, Users, Clock } from 'lucide-react';
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Erreur lors de la création du live');
      }
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {user && (
            <button
              ...
              className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-xl text-xs font-bold transition shadow-lg shadow-red-100"
            >
              <Plus size={14} /> Lancer un live
            </button>
          )}
        </div>
        ...
        {sessions.length > 0 ? (
          <div className="space-y-3 mb-6">
            {sessions.map(session => (
              <div key={session.id} className="bg-white border border-slate-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition">
                {/* Avatar hôte */}
                <div className="w-10 h-10 rounded-2xl bg-slate-100 overflow-hidden relative flex-shrink-0">
                  {session.hostAvatar
                    ? <Image src={session.hostAvatar} alt={session.hostName} fill className="object-cover" unoptimized={session.hostAvatar.startsWith('data:')} />
                    : <div className="w-full h-full flex items-center justify-center text-slate-400 font-bold text-sm">{session.hostName[0]}</div>
                  }
                </div>
                ...
                {/* Rejoindre */}
                <button onClick={() => handleJoin(session)} className="flex-shrink-0 flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-xs font-semibold transition">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  {shareMenuId === session.id && (
                    <div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 w-48">
                      <button onClick={() => handleCopyLink(session)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 text-xs text-slate-700 font-medium transition">
                        {copiedId === session.id
                      ...
                      <button onClick={() => handleShareWhatsApp(session)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-green-50 text-xs text-slate-700 font-medium transition">
                      ...
                      <button onClick={() => handleShareEmail(session)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-blue-50 text-xs text-slate-700 font-medium transition">
                ...
                {/* Terminer (hôte ou admin) */}
                {user && (user.uid === session.hostId || user.role === 'admin') && (
                  <button
                    onClick={async () => {
                      const headers = await getAuthHeaders();
                      await fetch('/api/liveSessions', { method: 'DELETE', headers, body: JSON.stringify({ id: session.id }) });
          ...
          <div className="bg-slate-50 border border-dashed border-slate-200 rounded-2xl py-6 text-center mb-6">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            <button onClick={() => setShowModal(false)} className="absolute top-5 right-5 text-slate-300 hover:text-slate-600"><X size={22} /></button>
                  ...
                  {SESSION_TYPES.map(t => (
                    <button
                      ...
                      className={`p-3 rounded-2xl border text-center transition ${
                        type === t.value ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-600 hover:border-slate-200'
                      }`}
                    >
