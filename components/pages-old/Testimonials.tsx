...
const Testimonials: React.FC<TestimonialsProps> = ({ testimonials: initialTestimonials, user, onAuthClick }) => {
  ...
  React.useEffect(() => { setTestimonials(initialTestimonials); }, [initialTestimonials]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingPost) {
      await fetch(`/api/testimonials/${editingPost.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify(postData) });
      setTestimonials(prev => prev.map(t => t.id === editingPost.id ? { ...t, ...postData } : t));
    } else {
      const res = await fetch('/api/testimonials', { method: 'POST', headers: headers(), body: JSON.stringify(postData) });
      const data = await res.json();
      const newT: Testimonial = { ...postData, id: data.id, votes: [], likes: [], dislikes: [], shares: 0, reposts: 0, comments: [], createdAt: new Date().toISOString() };
      setTestimonials(prev => [newT, ...prev]);
    }

    setShowAdd(false); setEditingPost(null); setNewTitle(''); setNewContent(''); setMediaData(null); setNewRating(5);
  };
  ...
  const handleLike = async (t: Testimonial) => {
    ...
    await fetch(`/api/testimonials/${t.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ likes: newLikes }) });
    setTestimonials(prev => prev.map(item => item.id === t.id ? { ...item, likes: newLikes } : item));
  };

  const handleShare = async (t: Testimonial) => {
    const newShares = (t.shares || 0) + 1;
    if (navigator.share) {
      try { await navigator.share({ title: t.title, text: t.content, url: window.location.href }); } catch {}
    } else { navigator.clipboard.writeText(window.location.href); alert("Lien copié !"); }
    await fetch(`/api/testimonials/${t.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ shares: newShares }) });
    setTestimonials(prev => prev.map(item => item.id === t.id ? { ...item, shares: newShares } : item));
  };
  ...
  const handleComment = async (t: Testimonial) => {
    ...
    await fetch(`/api/testimonials/${t.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ comments: newComments }) });
    setTestimonials(prev => prev.map(item => item.id === t.id ? { ...item, comments: newComments } : item));
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="text-center mb-16">
        ...
        <button onClick={() => { if (!user) { onAuthClick(); return; } setEditingPost(null); setNewTitle(''); setNewContent(''); setMediaData(null); setNewRating(5); setShowAdd(true); }} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-bold hover:bg-blue-600 transition shadow-xl">
          Contribuer au mur
        </button>
      </div>
          ...
          <div key={t.id} className="break-inside-avoid bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm group hover:shadow-xl transition-all relative overflow-hidden">
            {user && (user.uid === t.authorId || user.role === 'admin') && (
              <div className="absolute top-4 right-4 flex gap-2">
                <button onClick={() => { setEditingPost(t); setNewTitle(t.title ?? ''); setNewContent(t.content ?? ''); setNewRating(t.rating); if (t.media?.[0]) { setMediaData(t.media[0].url); setMediaType(t.media[0].type); } setShowAdd(true); }} className="p-2 text-slate-400 hover:text-blue-600 transition bg-white/80 rounded-full"><Edit2 size={16} /></button>
                <button onClick={() => handleDelete(t.id)} className="p-2 text-slate-400 hover:text-red-600 transition bg-white/80 rounded-full"><Trash2 size={16} /></button>
              </div>
            )}
            ...
            {t.media?.[0] && (
              <div className="rounded-3xl overflow-hidden mb-8 bg-slate-50 border border-slate-100 relative min-h-[200px]">
                {t.media[0].type === 'image' ? <Image src={t.media[0].url} alt="Media" width={800} height={300} className="w-full h-auto max-h-[300px] object-cover" unoptimized={t.media[0].url.startsWith('data:')} /> : <video src={t.media[0].url} controls className="w-full h-auto max-h-[300px]" />}
              </div>
            )}
            ...
            <div className="flex items-center gap-6 border-t border-slate-50 pt-6">
              <button onClick={() => handleLike(t)} className={`flex items-center gap-2 font-bold transition ${user && (t.likes || []).includes(user.uid) ? 'text-red-500' : 'text-slate-400 hover:text-red-500'}`}>
                <Heart size={18} className={user && (t.likes || []).includes(user.uid) ? 'fill-current' : ''} /><span className="text-xs">{(t.likes || []).length}</span>
              </button>
            ...
            {activeCommentPost === t.id && (
              <div className="mt-6 pt-6 border-t border-slate-50">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleComment(t)} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-xs">Envoyer</button>
                </div>
              </div>
            )}
          </div>
